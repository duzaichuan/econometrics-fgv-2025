...
- If the roots of $\phi(L)$ are outside the unit circle, then $\phi^{-1}(L)$ is well-defined;
- We can invert $\phi(L)$ to get:
$$y_t = \frac{\mu}{1 - \phi_1 - \ldots - \phi_p} +  \phi^{-1}(L)\theta(L) \varepsilon_t = \frac{\mu}{1 - \phi_1 - \ldots - \phi_p} + \psi(L) \varepsilon_t$$
...
plt.title('Simulated ARMA(1,4) Process')
plt.show()
```
...
- Remember that we can always factor the lag polynomials using their roots $r_{i}$ and $s_i$:
$$\phi(L) = (1 - r_1 L)(1 - r_2 L) \ldots (1 - r_p L)$$
$$\theta(L) = (1 + s_1 L)(1 + s_2 L) \ldots (1 + s_q L)$$
...
- Assume that $(1-\phi L)(y_t - \mu) = \varepsilon_t$ with $|\phi| < 1$;
...
- Main trick: use the equation above to back out a series of "estimated" shocks $\hat{\varepsilon}_t$;
- Assume that $\varepsilon_t = 0$ for $t \leq 0$ (before the sample);
